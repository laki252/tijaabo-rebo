...
def _convert_to_wav_chunks(input_path, chunk_length_ms=60000):
    ...
    try:
        for i in range(0, total_ms, chunk_length_ms):
            ...
            tmpfile = os.path.join(tmpdir, f"chunk{i//chunk_length_ms}.wav")
            chunk.export(tmpfile, format="wav")
            chunks.append(tmpfile)
    except Exception:
...
def transcribe_file(file_path: str, lang_code: str = "en") -> str:
    try:
        tmpdir, chunks = _convert_to_wav_chunks(file_path, chunk_length_ms=60000)
    except Exception as e:
