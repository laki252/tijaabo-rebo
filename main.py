    ...
    try:
        ...
        for chunk in chunks:
            ...
            if length_ms <= max_chunk_ms:
                ...
                # Tijaabada PCM: Kordhin ahaan u isticmaal .pcm, qaab ahaan u dhoofi .wav (waa PCM)
                chunk_file = os.path.join(DOWNLOADS_DIR, f"{os.path.basename(mp3_path)}_chunk{chunk_index}.pcm")
                ...
                while start < length_ms:
                    ...
                    sub.export(chunk_file, format="wav")
                    tmp_files.append(chunk_file)
                    start += step
        for i, chunk_file in enumerate(tmp_files, 1):
            
            # Tijaabo Cilmiyeed: Isticmaal sr.AudioFile oo leh fayl .pcm ah
