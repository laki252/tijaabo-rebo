...
def convert_to_wav(input_path: str, out_path: str):
    try:
        subprocess.run(["ffmpeg", "-y", "-i", input_path, "-ar", "16000", "-ac", "1", out_path], check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        return True
    except Exception:
        try:
...
def transcribe_file(file_path: str, lang_code: str = "en") -> str:
    ...
    if not converted:
        if os.path.exists(wav_path):
            os.remove(wav_path)
        raise RuntimeError("Failed to convert media to WAV for transcription")
    try:
