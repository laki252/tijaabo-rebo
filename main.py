...
def get_tts_user_voice(user_id: str) -> str:
    ...
    return settings.get("voice", "en-US-PhoebeMultilingualNeural") if settings else "en-US-PhoebeMultilingualNeural"
...
def help_handler(message):
    ...
    help_text = (
        "This bot converts your text into high-quality speech using Microsoft's Edge-TTS service.\n\n"
        "*How to Use:*\n"
        "1.  Use /start to select your desired language and voice.\n"
        "2.  Type any text and send it. The bot will reply with an audio file.\n\n"
        "*Available Commands:*\n"
        "/start - 🗣️ Change the voice actor\n"
        "/rate - ⚡️ Change the speech speed (-100 to +100)\n"
        "/pitch - 🎤 Change the voice pitch (-100 to +100)\n"
        "/voices_list - 📋 Get a full list of all available voices.\n"
...
def privacy_notice_handler(message):
    ...
    privacy_text = (
        "*Privacy Notice*\n\n"
        "This bot is designed to be privacy-friendly.\n\n"
        "1.  *What we process:* To generate speech, your text message is sent to Microsoft's TTS service. We do not read or store your text messages after the audio is generated.\n"
        "2.  *What we store:* We only store your numeric Telegram User ID to remember your voice, pitch, and rate settings for your convenience.\n"
        "3.  *What we DON'T store:* We do not store your name, username, phone number, or the content of your text messages.\n"
        "4.  *Audio Files:* Audio files are generated, sent to you, and then immediately deleted from our server. They are not saved."
    )
