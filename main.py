        ...
        with audio_data as source:
            ...
            while True:
                ...
                try:
                    ...
                    # 👈 ISBEDDELKA WUXUU KU YIMID HALKAN: Waxaan ka beddelnay r.record(source, duration=40)
                    # una beddelnay r.listen(source, phrase_time_limit=55) si loo hubiyo in hadalka uusan dhexda ka jarin marka uu dhamaado
                    # wakhtiga. Tani waxay duubeysaa ilaa ay aamusnaan ka timaaddo.
                    chunk = r.listen(source, phrase_time_limit=55)
                    
                except Exception as e:
                    logging.info(f"Qalad r.listen, malaha dhamaadka file-ka: {e}")
                    break
                
                if not chunk.get_raw_data():
                    logging.info("Dhamaadka file-ka waa la gaaray (chunk maran).")
                    break
                
                try:
                    text_part = r.recognize_google(chunk, language=lang_code)
                    full_text += text_part + " "
                except sr.UnknownValueError:
                    logging.info("Chunk unrecognized")
                except sr.RequestError as e:
                    logging.error(f"Chunk request failed: {e}")
